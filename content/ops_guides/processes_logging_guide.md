...
This directory can also be used to derived each core service's SystemD init name. For instance, enumerating /etc/sysconfig will show a file called cloudify-mgmtworker. This is the name of the service, and thus to query the service status can be done using the command service cloudify-mgmtworker status.
