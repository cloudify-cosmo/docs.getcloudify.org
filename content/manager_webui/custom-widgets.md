...
* `backend.js` - A widget backend allowing to use backend services from widget's frontend code (optional)
...
You place the widget in the widgets library, which must hold the ID of the widget. For example, if you are creating a blueprint (ID is `blueprint`), the blueprint library will be:
```
/widgets   
   /blueprint   
      widget.js
