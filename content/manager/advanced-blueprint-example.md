...
In this tutorial we will create a [blueprint]({{ terminology_link }}#blueprint) that describes
the [topology]({{ terminology_link }}#topology) of the Nodecellar [application]({{ terminology_link }}#application).
We will write, step by step, a simplified version of the [Single Host Nodecellar]({{ singlehost_nodecellar_url }}) blueprint. <br>
...
- [Nodecellar]({{ nodecellar_link }}) - This is the application business logic packed as a Node.JS application.

The topology looks like this:

![nodecllar app]({{ c.img("guide/quickstart/nodecellar_singlehost_topology.png" ) }})
...
- [plugins]({{ terminology_link }}#plugin)
- [node_types]({{ terminology_link }}#type)
- [relationships]({{ terminology_link }}#relationship-type)

For a complete reference of these constructs refer to the DSL Specification section.

In our case, we need to import Cloudify's [built-in types definitions]({{ types_yaml_link }}) since our custom `node_types` will *derive from* some of these types.
...
{% call c.note("Note") %}
**All of the paths specified are relative to the blueprint file directory. You can find the scripts [here]({{ nodecellar_scripts_url }})**
...
Cloudify's [built-in types definitions]({{ types_yaml_link }}) comes with a relationship of type `cloudify.relationships.connected_to`, which seems to fit into our case.
...
Cloudify's [built-in types definitions]({{ types_yaml_link }}) comes with a relationship of type `cloudify.relationships.contained_in`, which also seems appropriate.
...
This [script]({{ nodecellar_scripts_url }}/mongo/set-mongo-url.sh) uses the [Context API]({{ terminology_link }}#context-object)
to set [runtime properties]({{ terminology_link }}#runtime-properties) that determine the MongoDB URL on the *source* node of this relationship.
...
This type is one that is provided by Cloudify's [built-in types definitions]({{ types_yaml_link }}).
...
- *install_agent* - This tells cloudify whether or not to install a Cloudify [Agent]({{ agent_overview_link }}) on this host.
