...
The [blueprint]({{ blueprint_file_link }}) that you'll be deploying describes a nodejs application that connects to a MongoDB database and presents a wine catalog.
...
* [Oracle VirtualBox]({{ virtualbox_link }}) (this box has been tested with version 4.3 or higher, but earlier versions should work as well).
* [Vagrant]({{ vagrant_link }}) (Make sure that you are using version 1.5 or above!).
* At least 2GB of free RAM

{% call c.note("Running inside a VM") %}
...
{% call c.note("Notes for Windows users") %}
...
{% call c.collapse("Step 1: Download Vagrantfile and run VM", open = true) %}
...
Download this [Vagrantfile]({{ vagrant_file_link }}) to your local directory. Then, run this command:
...
{% call c.collapse("Step 2: SSH to the Vagrant Box and Connect to the Running Manager") %}
...
{% endcall %}
{% call c.collapse("Step 3: Download the blueprint") %}
...
git checkout tags/{{ nodecellar_version }}
```
{% endcall %}
{% call c.collapse("Step 4: Upload the Blueprint and Create a Deployment") %}
...
![Nodecellar Blueprint]({{ c.img("guide/quickstart/nodecellar_singlehost_topology.png" ) }})

This blueprint defines some input parameters:

![Nodecellar Inputs]({{ c.img("guide/quickstart/nodecellar_singlehost_inputs.png" ) }})
...
![Nodecellar Deployment]({{ c.img("guide/quickstart/nodecellar_deployment.png" ) }})

{% endcall %}
{% call c.collapse("Step 5: Install the Deployment") %}
...
![Events]({{ c.img("guide/quickstart/events.png" ) }})
...
{% endcall %}
{% call c.collapse("Step 6: Test Drive the Application") %}
...
![Nodecellar]({{ c.img("guide/quickstart/nodecellar.png" ) }})

{% endcall %}
{% call c.collapse("Step 7: Uninstall the Deployment") %}
...
{% endcall %}
{% call c.collapse("Step 8: Delete the Deployment") %}
...
{% endcall %}
{% call c.collapse("Step 9: Tear down the Manager") %}
