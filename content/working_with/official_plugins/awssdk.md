...
Note the following example node template that creates a VM, using the node type [cloudify.nodes.aws.ec2.Instances](https://docs.cloudify.co/4.3.0/working_with/official_plugins/awssdk/#cloudify-nodes-aws-ec2-instances).

Based on the documentation for that node type, note see that the `resource_config` is based on the boto method [run_instances](http://boto3.readthedocs.io/en/latest/reference/services/ec2.html#EC2.Client.run_instances).
