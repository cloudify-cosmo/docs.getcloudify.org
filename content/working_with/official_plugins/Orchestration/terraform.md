...
**Workflows**
  ...
  * `reload_terraform_template`: execute terraform reload operation on all terraform.Module node instances
...
To execute terraform reload workflow :

```bash
cfy executions start reload_terraform_template -d {deployment_id} -p '{"source": "/tmp/aws-two-tier.zip","destroy_previous":"true","node_ids": ["cloud_resources"]}'
```
