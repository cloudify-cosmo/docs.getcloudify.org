...
It is possible to cancel an execution whose [status]({{< relref "operations/workflows/statuses.md" >}}) is either `pending` or `started`.
...
To use force-cancellation instead, simply add the `force` flag. For a syntax reference, see the [CLI commands reference]({{< relref "cli/executions.md" >}}).
