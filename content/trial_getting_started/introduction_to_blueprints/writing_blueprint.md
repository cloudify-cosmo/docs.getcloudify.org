...
Cloudify Blueprint is a YAML file with definitions of resources and connections between them, the syntax is based on the TOSCA  specification (Topology Orchestration Specification for Cloud Applications).  It is best suited to  enable multi-domain service orchestration based on Ansible, Terraform, AWS Cloud Formation, Azure ARM, Kubernetes, Helm, etc...
...
* Step 3 - Integrate between resources in a blueprint, and use relationships to make the resources work together.
...
If you are interested to learn how to use the Cloudify blueprint to run  Ansible, Terraform, and Kubernetes based services across a  Multi-Cloud  infrastructure refer to the relevant section in the Cloudify [Getting Started guide](https://docs.cloudify.co/latest/trial_getting_started/)
...
The [input section](https://docs.cloudify.co/latest/developer/blueprints/spec-inputs/) as its name suggests defines the arguments that a user can pass and are needed to run the blueprint. An input can have a default value which will be used in the case that the user didn’t provide input on this input field. Input can also be constrained to ensure that the value provided is valid.

#### The capabilities section

The [capabilities](https://docs.cloudify.co/latest/developer/blueprints/spec-capabilities/) section exposes the relevant resources to the outside world. Usually, that will be the API endpoint, the website URL that is needed to get access to this service by other users or services.
...
The create and delete operation points to install.py and uninstall.py respectively which are in this case local python scripts located in the same folder where the blueprint.yaml is located (the path to those scripts is relative to the location of the blueprint location).
...
The Certificate and WebService are ServiceComponent node types each point to a child blueprint respectively..
The difference between a regular node type and a service component node type is the lego-icon on the top right. The lego icon indicates that this specific node points to another blueprint. At the bottom left we can see an arrow and cross that allows us to zoom in and out  and see the underlying blueprint behind each component.
...
There are basically two modes in which this could be done:
...
```
4. Example 4 - a nested blueprint
```
...
* [Creating a custom plugin](https://docs.cloudify.co/latest/developer/writing_plugins/).
