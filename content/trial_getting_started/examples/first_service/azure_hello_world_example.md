...
This Example demonstrates a simple topology setup in **Azure**,
the deployment consists of:

 * VM
 * Simple Web Server + Simple Page
 * All of the essential peripherals in Azure (security group, network, etc.)
...
The hello-world blueprint is available [here](https://github.com/cloudify-community/blueprint-examples/blob/master/hello-world-example/azure.yaml).
...
```bash
docker exec -it cfy_manager_local sh -c "cfy install https://github.com/cloudify-community/blueprint-examples/releases/download/5.0.5-40/hello-world-example.zip -n azure.yaml "
```
...
```
docker exec -it cfy_manager_local sh -c "cfy executions start uninstall -d hello-world-example.azure -p ignore_failure=true"
...
Select the deployment (hello-world-example.azure) and explore the topology, inputs, outputs, nodes, and logs.
...
we will go to the Cloudify examples repository in github and examine it there: [azure.yaml](https://github.com/cloudify-community/blueprint-examples/blob/master/hello-world-example/azure.yaml).
...
`3`. To upload the required plugins go to **Cloudify Catalog** and upload the plugins you need to use
     (for this example azure-plugin, ansible-plugin and utilities-plugin are needed).
