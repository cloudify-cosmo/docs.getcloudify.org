...
1. Start the image in your platform:
    ...
    * Docker
        ...
        `sudo docker load -i [tar file image]`
...
1. To use Cloudify Manager from the terminal using [Cloudify CLI]({{< relref "install_maintain/installation/installing-cli.md" >}}), run the following command with your instance details.
    ...
    * password - `admin`
    ...
    Because the `cfy` command is already available and configured, you can navigate to Cloudify Manager using SSH and use the already configured CLI (this will not work for Docker). You can also install [Cloudify CLI]({{< relref "install_maintain/installation/installing-cli.md" >}}) on a local host and connect to the instance remotely.
