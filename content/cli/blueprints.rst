...
The ``cfy blueprints`` command is used to manage blueprints on a
Cloudify Manager instance.

You can use the command to upload, delete, download, validate and list
blueprints and to retrieve information for a specific blueprint.
...
-  ``-b, --blueprint-id=BLUEPRINT_ID`` - The unique identifier for the
   blueprint

-  | ``-n, --blueprint-filename TEXT`` -
   | The name of the archive’s main blueprint file. Only relevant if
     uploading an archive.

-  | ``--validate`` -
   | Validate the blueprint before uploading it to the manager

-  | ``-t --tenant-name TEXT`` -
   | The name of the tenant of the blueprint. If not specified, the
...
Delete a blueprint. It’s important to note that deleting a blueprint
does not delete the deployments created from that blueprint and
resources of those deployments.
...
``BLUEPRINT_PATH`` - The path to the blueprint yaml or to the directory
in which the blueprint yaml files resides.
...
Validate a blueprint. Checks that the blueprint’s syntax is valid and
that all imports are accessible.

.. note::
    :class: summary

    Import validation is done only on the client    side. That means that if, for some reason, the imports are accessible by
    the client but not on the manager, this validation will still pass. {{%
...
``BLUEPRINT_PATH`` - The path to the blueprint for which the file will
be generated.
...
-  ``-t --tenant-name TEXT`` The name of the tenant for which to list
   the blueprints. If not specified, the current tenant is used. This
...
``BLUEPRINT_ID`` - The ID of the blueprint for which to retrieve
information.
...
-  ``-t --tenant-name TEXT`` The name of the tenant for which to
   retrieve the blueprint information. If not specified, the current
   tenant is used.
...
``BLUEPRINT_ID`` - The path of the blueprint for which to retrieve
inputs.
...
-  ``-t --tenant-name TEXT`` The name of the tenant from which to
   retrieve the blueprints. If not specified, the current tenant is
   used.
