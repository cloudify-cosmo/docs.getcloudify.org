...
The ``cfy node-instances`` command is used to view information about the
different node-instances of a deployment.

You can use the command to list the node-instances of a specific
deployment or of all deployments, and to retrieve information about a
...
If ``DEPLOYMENT_ID`` is provided, lists node-instances for that
deployment. Otherwise, lists node-instances for all deployments.
...
``NODE_INSTANCE_ID`` is the ID of the node-instance for which to
...
-  ``-t, --tenant-name TEXT`` The name of the tenant of the
   node-instance. If unspecified, the current tenant is used.
