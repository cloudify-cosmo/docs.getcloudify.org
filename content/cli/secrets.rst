...
The ``cfy secrets`` command is used to manage Cloudify secrets
(key-value pairs).
...
-  ``-s, --secret-string TEXT`` - The string to use as the secret’s
   value.
-  ``-f, --secret-file TEXT`` - The name of the secret file that
...
-  ``-u, --update-if-exists`` - Update secret with new value if it
   already exists
...
-  ``-t, --tenant-name TEXT`` - The name of the tenant from which to
   list secrets. If unspecified, the current tenant is used. This
